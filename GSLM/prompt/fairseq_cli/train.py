...
# fairseq_usr #
# =========== #
from fairseq_usr.trainer import PromptTrainer as Trainer
...
def dump_args(input_args: list, f_path: Path):
    now = datetime.now()
...
# [SpeechPrompt]: modified cli_main
def cli_main(parser, input_args) -> None:
    # args = options.parse_args_and_arch(parser, input_args, modify_parser=modify_parser)
    args = options.parse_args_and_arch(parser, input_args, modify_parser=None)

    dump_args(input_args=input_args, f_path=Path(args.save_dir) / "args_input.txt")
