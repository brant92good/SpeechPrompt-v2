...
def get_feature_iterator(
    ...
    with open(manifest_path, "r") as fp:
        ...
        if len(lines[0].split("\t")) > 2:
            ...
            file_path_list = [
                (os.path.join(root, line.split("\t")[0]), line.split("\t")[2], line.split("\t")[3])
                for line in lines
                if len(line) > 0
            ]
        else:
        ...
        def iterate():
            if type(file_path_list[0]) == str:
                for file_path in file_path_list:
                    feats = reader.get_feats(file_path)
                    yield feats.cpu().numpy()
            else:
                # for freesound preprocessing
                for (file_path, offset, duration) in file_path_list:
                    feats = reader.get_feats(file_path, offset, duration)
                    yield feats.cpu().numpy()
